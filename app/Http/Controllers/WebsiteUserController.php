...
class WebsiteUserController extends \App\Http\Controllers\Controller
{
    ...
    {
        $website_user = Websites::where('unique_id', $request->uuid)->first();
    ...
    public function isWebsiteUserValid(Request $request)
    {
        $website= WebsiteUsers::where('unique_id', $request->website_id)->first();
        if (!$website) {
            return response()->json(['success' => false, 'message' =>'Invalid Website ID']);
        } else {
